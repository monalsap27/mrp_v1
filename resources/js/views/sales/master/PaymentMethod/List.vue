        ...
        <template slot-scope="scope">
          ...
          >
            <el-button
              ...
              size="small"
              icon="el-icon-edit"
              @click="handleEditPaymentMethod(scope.row.id)"
            />
          </el-tooltip>
    ...
    >
      <div v-loading="paymentMethodCreating" class="form-container">
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="createPaymentMethod()">
            {{ $t('table.confirm') }}
          </el-button>
        </div>
      </div>
    ...
    >
      <div v-loading="paymentMethodEdit" class="form-container">
        ...
        >
          ...
          <el-form-item :label="$t('Code')" prop="code">
            <el-input
              v-model="currentPaymentMethod.code"
              placeholder="type code here"
            />
          </el-form-item>
        </el-form>
...
import {
  ...
  DeletePaymentMethod,
} from '@/api/sales/master/payment_method';
  ...
  methods: {
    ...
    createPaymentMethod() {
      this.$refs['paymentMethodForm'].validate((valid) => {
        console.log(valid);
        if (valid) {
          this.newPaymentMethod.roles = [this.newPaymentMethod.role];
          this.paymentMethodCreating = true;
          createOrUpdatePaymentMethod(this.newPaymentMethod)
            .then((response) => {
              this.$message({
                message:
                  'New province ' +
    ...
    async handleEditPaymentMethod(id) {
      this.currentId = id;
      this.paymentMethodEdit = true;
      this.dialogEditVisible = true;
      const foundPaymentMethod = this.list.find(
      ...
      this.currentPaymentMethod = {
        ...
        provinces: foundPaymentMethod.m_provinces_id,
      };
      this.paymentMethodEdit = false;
    },
    updatePaymentMethod() {
      this.paymentMethodEdit = true;
      createOrUpdatePaymentMethod(this.currentPaymentMethod)
        .then((response) => {
          this.$message({
            message:
              'Payment Method ' +
